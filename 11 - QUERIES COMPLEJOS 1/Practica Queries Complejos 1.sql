-- Práctica de queries complejas 1
...
-- 2.
-- a Se requiere crear una tabla temporal #ABC_Productos un ABC de Productos ordenado por cantidad de venta en u$, los datos solicitados son:
...
WHERE 
    i.manu_code IN (SELECT s2.manu_code
FROM products s2
...
-- 2.
-- b Listar los datos generados en la tablas #ABC_Productos ordenados en forma descendente por 'u$ por Producto'
-- y en forma ascendente por stock_num y manu_code.

SELECT *
FROM #ABC_Productos
order by 'u$ por producto' desc, stock_num, manu_code


-- 3.
-- En función a la tabla temporal generada en el punto 2, obtener un listado que detalle para cada tipo de producto existente
...
-- 4.
-- Dado los productos con nro de stock 5, 6 y 9 del fabricante 'ANZ' listar de a pares los clientes que hayan solicitado 
...
-- AND c1.customer_num != c2.customer_num) 
WHERE 
