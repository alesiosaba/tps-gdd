...
CREATE VIEW vrecu1
AS
    SELECT m.manu_code, m.manu_name, count(stock_num) cant_productos, (SELECT max(order_date)
        FROM orders o JOIN items i ON o.order_num=i.order_num AND i.manu_code=m.manu_code) ult_compra
...
CREATE VIEW vrecu1
AS
    SELECT m.manu_code, m.manu_name, count(distinct s.stock_num) cant_productos, max(o.order_date)ult_compra
    FROM manufact m LEFT JOIN products s ON s.manu_code = m.manu_code LEFT JOIN items i ON s.manu_code = i.manu_code AND s.stock_num=i.stock_num LEFT JOIN orders o ON i.order_num = o.order_num
    GROUP BY m.manu_code,m.manu_name
    HAVING count(distinct s.stock_num)=0 OR count(distinct s.stock_num)>2
GO
-- Opción 3 
GO
CREATE VIEW vrecu1
AS
    SELECT m.manu_code, m.manu_name, count(distinct s.stock_num) cant_productos, max(o.order_date)ult_compra
    FROM manufact m LEFT JOIN products s ON s.manu_code = m.manu_code LEFT JOIN items i ON s.manu_code = i.manu_code AND s.stock_num=i.stock_num LEFT JOIN orders o ON i.order_num = o.order_num
    WHERE m.manu_code IN (SELECT m2.manu_code
    FROM manufact m2 JOIN products s2 ON (m2.manu_code = s2.manu_code)
...
select
    ...
    cant_productos,
    case
        when ult_compra is null then 'No posee Productos'
        when ult_compra is not null then cast(ult_compra as char)
    end
from
    vrecu1
-- Opcion 2 con COALESCE -- falla por problemas de Casteo select manu_code,manu_name,cant_productos, COALESCE(ult_compra,'No posee Productos') ultcompra from vrecu1
...
HAVING
    SUM(unit_price * quantity) > (
        select
    SUM(unit_price * quantity) / count(DISTINCT i.manu_code)
from
    items i
    )
ORDER BY
...
UNION
    ...
    WHERE i2.stock_num IN (SELECT stock_num
    FROM products
    GROUP BY stock_num
    HAVING count(*) >2))
...
SELECT top 5
    t.description, c.state, SUM(i.quantity)
...
UNION
    ...
    WHERE c.customer_num NOT IN (SELECT customer_num
    FROM orders)
...
WHERE o.order_num = (SELECT MAX(order_num)
    ...
    OR c.customer_num NOT IN (SELECT customer_num
    FROM orders)
...
having sum(quantity) = (select top 1
    sum(i2.quantity)
from items i2
where i2.stock_num = i.stock_num
